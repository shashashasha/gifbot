...
var uploadCard = function(card) {
  ...
  var local_file = './processing/' + card.job_name + '/' + card.folder + '/' + card.zip_filename,
    local_image0 = './processing/' + card.job_name + '/' + card.folder + '/000.jpg',
    local_image1 = './processing/' + card.job_name + '/' + card.folder + '/001.jpg',
    destination = getCurrentUploadFolder() + card.zip_filename,
    destination_image = getCurrentUploadFolder() + card.zip_filename.replace('.zip', ''),
    destination_url0 = getCurrentUploadFolder() + card.zip_filename.replace('.zip', '_000.jpg'),
    destination_url1 = getCurrentUploadFolder() + card.zip_filename.replace('.zip', '_001.jpg');

  card.zip_url = 'http://' + config.S3Bucket + '/' + destination;
  card.url0 = 'http://' + config.S3Bucket + '/' + destination_image + '_000.jpg';
  card.url1 = 'http://' + config.S3Bucket + '/' + destination_image + '_001.jpg';

  console.log('>>>> uploading to s3:\t', destination);
  console.log('>>>> uploading to s3:\t', card.url0);
  console.log('>>>> uploading to s3:\t', card.url1);

  if (DEBUG) {
    console.log('>>>> skipping upload because DEBUG');
    deferred.resolve(card);
  } else {
    // upload flip images then zip
    s3.putFile(local_image0, destination_url0, function(err, response) {
        if (!err) {
          console.log('>>>> uploaded!\t\t', card.url0, '1st image');
          s3.putFile(local_image1, destination_url1, function(err, response) {
              if (!err) {
                console.log('>>>> uploaded!\t\t', card.url1, '2nd image');
